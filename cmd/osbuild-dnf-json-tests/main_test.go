...
func TestDepsolvePackageSets(t *testing.T) {
	...
	// Set up temporary directory for rpm/dnf cache
	dir := t.TempDir()
	solver := dnfjson.NewSolver(cs9.ModulePlatformID(), cs9.Releasever(), platform.X86_64ArchName, cs9.Name(), dir)

	repos, err := rpmmd.LoadRepositories([]string{repoDir}, cs9.Name())
	require.NoErrorf(t, err, "Failed to LoadRepositories %v", cs9.Name())
	x86Repos, ok := repos[platform.X86_64ArchName]
	require.Truef(t, ok, "failed to get %q repos for %q", platform.X86_64ArchName, cs9.Name())

	x86Arch, err := cs9.GetArch(platform.X86_64ArchName)
	require.Nilf(t, err, "failed to get %q arch of %q distro", platform.X86_64ArchName, cs9.Name())

	qcow2ImageTypeName := "qcow2"
	qcow2Image, err := x86Arch.GetImageType(qcow2ImageTypeName)
	require.Nilf(t, err, "failed to get %q image type of %q/%q distro/arch", qcow2ImageTypeName, cs9.Name(), platform.X86_64ArchName)

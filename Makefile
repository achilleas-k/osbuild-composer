...
# DEVELOPMENT TARGETS
#
# The following targets are useful for development and testing.
#

#
# Build
#
# The following rules are for building binaries and autogenerated code where
# necessary.
#

GOSOURCES = $(shell find . -type f -iname "*.go") go.sum go.mod
OPENAPISPECS = $(shell find . type f -iname "*openapi*.yml")

.PHONY: prepare-source
prepare-source: $(GOSOURCES) ./tools/prepare-source.sh
	./tools/prepare-source.sh

#
# Test and coverage
#
# The following rules run the Go unit tests.  They all depend on the Go file
# sources so they wont be rerun if nothing changed.  A test coverage profile is
# created with the test command.  The file can be converted to html with the
...
coverage: $(GOSOURCES)
	go test -covermode=count -coverpkg=./... -coverprofile=coverage ./...

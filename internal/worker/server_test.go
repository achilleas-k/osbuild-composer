...
func enqueueAndFinishTestJobDependencies(s *worker.Server, deps []testJob) ([]uuid.UUID, error) {
	...
	for _, dep := range deps {
		...
		case *worker.OSBuildJob:
			...
			id, err = s.EnqueueOSBuildAsDependency(platform.X86_64ArchName, job, depUUIDs, "")
			if err != nil {
				return nil, err
			}
		...
		_, token, _, _, _, err := s.RequestJobById(context.Background(), platform.X86_64ArchName, id)
		if err != nil {
			return nil, err
		}

	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			worker.MockExecCommand(func(name string, arg ...string) *exec.Cmd {
				...
				// The blueprint path is a under random temporary directory, so
				...
				if bpPathIdx > 0 {
					...
					// make sure the blueprint path in the call at least ends in blueprint.json
					assert.True(strings.HasSuffix(bpPath, "/blueprint.json"))
				...
				// The repositories path is under a random temporary directory,
				// so let's search for it and replace the path in the expected
				// args
				...
				if dataDirIdx > 0 {
					...
					// load the repo file and compare it with the original

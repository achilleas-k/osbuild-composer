...
func TestDepsolver(t *testing.T) {
	...
	deps, err := solver.Depsolve([]rpmmd.PackageSet{pkgset},
		[][]rpmmd.RepoConfig{{s.RepoConfig}})
	if err != nil {
		t.Fatal(err)
	}
...
func expectedResult(url string) []DepsolveResult {
	...
	for idx := range exp[0].Dependencies {
		urlTemplate := exp[0].Dependencies[idx].RemoteLocation
		exp[0].Dependencies[idx].RemoteLocation = fmt.Sprintf(urlTemplate, url)
	}
	return exp
}

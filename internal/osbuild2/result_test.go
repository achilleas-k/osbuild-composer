...
`
	...
	assert.Equal(t, result.Pipelines[0].Stages[0].Type, "org.osbuild.rpm")
	metadata, ok := result.Pipelines[0].Stages[0].Metadata.(*RPMStageMetadata)
	assert.True(t, ok)
...
func TestWriteFull(t *testing.T) {
	...
	testPipeline := PipelineResult{
		Runner: "org.osbuild.rhel84",
		Name:   "build",
		Stages: []StageResult{dnfStage, testStage},
	}

	testComposeResult := Result{
		TreeID:    "TreeID",
		OutputID:  "OutputID",
		Pipelines: []PipelineResult{testPipeline},
		Success:   true,
	}

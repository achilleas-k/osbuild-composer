...
func TestGenFileNodesStages(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			files: []*fsnode.File{
				ensureFileCreation(fsnode.NewFile("/etc/file", []byte(fileData1), nil, nil, nil)),
			},
			expected: []*Stage{
		...
		},
		{
			...
			files: []*fsnode.File{
				ensureFileCreation(fsnode.NewFile("/etc/file", []byte(fileData1), nil, nil, nil)),
				ensureFileCreation(fsnode.NewFile("/etc/file2", []byte(fileData2), nil, nil, nil)),
			},
			expected: []*Stage{
				NewCopyStageSimple(&CopyStageOptions{
					Paths: []CopyStagePath{
		...
		},
		{
			...
			files: []*fsnode.File{
				ensureFileCreation(fsnode.NewFile("/etc/file", []byte(fileData1), common.ToPtr(os.FileMode(0644)), "root", int64(12345))),
				ensureFileCreation(fsnode.NewFile("/etc/file2", []byte(fileData2), common.ToPtr(os.FileMode(0755)), int64(12345), "root")),
			},
			expected: []*Stage{
				NewCopyStageSimple(&CopyStageOptions{
					Paths: []CopyStagePath{

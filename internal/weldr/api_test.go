...
func createWeldrAPI(tempdir string, fixtureGenerator rpmmd_mock.FixtureGenerator, dataGenerator func(string) string) (*API, *store.Store) {
	...
	if dataGenerator != nil {
		// configure mock-dnf-json
		testCasePath, err := os.MkdirTemp(tempdir, "")
		if err != nil {
			panic(err)
		}
		testCase := dataGenerator(testCasePath)
		solver.SetDNFJSONPath("go", "run", "../../cmd/mock-dnf-json", testCase)
	}
...
func TestBasic(t *testing.T) {
	...
	api, _ := createWeldrAPI(t.TempDir(), rpmmd_mock.BaseFixture, nil)
	for _, c := range cases {
...
func TestBlueprintsNew(t *testing.T) {
	...
	for _, c := range cases {
		api, _ := createWeldrAPI(tempdir, rpmmd_mock.BaseFixture, nil)
...
func TestBlueprintsFreeze(t *testing.T) {
	t.Run("json", func(t *testing.T) {
		...
		for _, c := range cases {
			api, _ := createWeldrAPI(tempdir, c.Fixture, nil)
...
func TestBlueprintsDepsolve(t *testing.T) {
	var cases = []struct {
		Fixture        rpmmd_mock.FixtureGenerator
		DataGenerator  func(string) string
		ExpectedStatus int
		ExpectedJSON   string
	}{
		{rpmmd_mock.BaseFixture, dnfjson_mock.BaseDepsolve, http.StatusOK, `{"blueprints":[{"blueprint":{"name":"test","description":"Test","distro":"","version":"0.0.1","packages":[{"name":"dep-package1","version":"*"}],"groups":[],"modules":[{"name":"dep-package3","version":"*"}]},"dependencies":[{"name":"dep-package3","epoch":7,"version":"3.0.3","release":"1.fc30","arch":"x86_64","check_gpg":true},{"name":"dep-package1","epoch":0,"version":"1.33","release":"2.fc30","arch":"x86_64","check_gpg":true},{"name":"dep-package2","epoch":0,"version":"2.9","release":"1.fc30","arch":"x86_64","check_gpg":true}]}],"errors":[]}`},
		{rpmmd_mock.NonExistingPackage, dnfjson_mock.NonExistingPackage, http.StatusOK, `{"blueprints":[{"blueprint":{"name":"test","description":"Test","distro":"","version":"0.0.1","packages":[{"name":"dep-package1","version":"*"}],"groups":[],"modules":[{"name":"dep-package3","version":"*"}]},"dependencies":[]}],"errors":[{"id":"BlueprintsError","msg":"test: DNF error occurred: MarkingErrors: Error occurred when marking packages for installation: Problems in request:\nmissing packages: fash"}]}`},
		{rpmmd_mock.BadDepsolve, dnfjson_mock.BadDepsolve, http.StatusOK, `{"blueprints":[{"blueprint":{"name":"test","description":"Test","distro":"","version":"0.0.1","packages":[{"name":"dep-package1","version":"*"}],"groups":[],"modules":[{"name":"dep-package3","version":"*"}]},"dependencies":[]}],"errors":[{"id":"BlueprintsError","msg":"test: DNF error occurred: DepsolveError: There was a problem depsolving ['go2rpm']: \n Problem: conflicting requests\n  - nothing provides askalono-cli needed by go2rpm-1-4.fc31.noarch"}]}`},
	}
	...
	for idx, c := range cases {
		...
		api, _ := createWeldrAPI(tempdir, c.Fixture, c.DataGenerator)
...
func TestOldBlueprintsUndo(t *testing.T) {
	api, _ := createWeldrAPI(t.TempDir(), rpmmd_mock.OldChangesFixture, nil)
	rand.Seed(time.Now().UnixNano())
	// math/rand is good enough in this case
	/* #nosec G404 */
	ignoreFields := []string{"commit", "timestamp"}
...
func TestCompose(t *testing.T) {
	...
	for _, c := range cases {
		api, s := createWeldrAPI(tempdir, rpmmd_mock.NoComposesFixture, nil)
...
func TestComposeDelete(t *testing.T) {
	...
	for _, c := range cases {
		api, s := createWeldrAPI(tempdir, rpmmd_mock.BaseFixture, nil)
